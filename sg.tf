...
/*Q. backup 서버로 파일을 보낼거면 10.100.0.0 번대 22번 포트 모두 허용해줘야 하는가? */
# case 1. ec2에서 백업 서버로 파일을 보낼 때
# - 보안 그룹은 stateful이라서 인바운드 없이도 됨. 

# case 2. 백업 서버가 파일을 직접 가져올 때
# - 접속이 필요하므로, 22번 허용 필요
# ==> 결론적으로 '접속을 어느쪽에서 시작'하는지에 따라 보안그룹 설정이 필요할 수도 아닐 수도. 
...
resource "aws_security_group" "internal_dns_sg" {
  ...
  ingress {
    ...
    #cidr_blocks     = [var.my_public_ip]  # 보안그룹 적을 땐 security_groups. 퍼블릭에 뒀을 경우 내 IP허용
    description     = "allow me ssh"
  }
  ingress {
    from_port       = 53  
...
resource "aws_security_group" "vpn_sg" {
  ...
  # Q. bastion 만 허용? 아니면 그냥 내 아이피 허용? 
  ingress {
    from_port       = 22  
    to_port         = 22
    protocol        = "tcp"
    security_groups = [ aws_security_group.bastion_sg.id ]  # --> 보안 그룹을 지정할 때는 security_groups를 사용
    #cidr_blocks = [var.my_public_ip]  # public에 둘 때 사용. 보안그룹 적을 땐 security_groups
